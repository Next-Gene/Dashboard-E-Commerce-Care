...
export class CategoryService {
  ...
  addCategory(data: {
    ...
    return this._httpClient
      .post<{ message: string; data: { id: number } }>(
  ...
  uploadPhotoById(id: string, formData: FormData): Observable<any> {
    return this._httpClient
      .post(`${ApiEndpoint.CATEGORY_PHOTO}/${id}/photo`, formData)
      .pipe(tap((response) => console.log('Photo Upload Response:', response)));
  }

  // Get single category by ID
  getCategoryById(id: string): Observable<Category> {
    return this._httpClient
      .get<Category>(`${ApiEndpoint.CATEGORIES}/${id}`)
      .pipe(
